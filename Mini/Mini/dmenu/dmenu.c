...
static Bool selectonly = False;
static Bool nocaret = False;
...
void parse_rc_line(char* line)
{
    KVPair* kvp = rc_parseline(line);
    if (kvp == NULL)
	return;
    
    char* key = kvp->key;
    int i_val = atoi(kvp->val);
    double f_val = atof(kvp->val);

    if (!strcmp(key, "TopBar"))
	topbar = !!i_val;
    else if (!strcmp(key, "History"))
	histfile = strdup(kvp->val);
    else if (!strcmp(key, "Quiet"))
	quiet = !!i_val;
    else if (!strcmp(key, "Fast"))
	fast = !!i_val;
    else if (!strcmp(key, "Fuzzy"))
	match = i_val ? matchfuzzy : matchstr;
    else if (!strcmp(key, "FilterMode"))
	filter = !!i_val;
    else if (!strcmp(key, "CaseInsensitive"))
    {
	if (i_val)
	{
	...
	dynheight = !!i_val;
    else if (!strcmp(key, "MatchToken"))
	match = i_val ? matchtok : matchstr;
    else if (!strcmp(key, "XPosition"))
	xoffset = i_val;
    else if (!strcmp(key, "YPosition"))
	yoffset = i_val;
    else if (!strcmp(key, "WindowWidth"))
	width = i_val;
    else if (!strcmp(key, "Lines"))
	lines = i_val;
    else if (!strcmp(key, "LineHeight"))
	line_height = i_val;
    else if (!strcmp(key, "UnderlineHeight"))
	under_height = i_val;
    else if (!strcmp(key, "Screen"))
	snum = i_val;
    else if (!strcmp(key, "WindowName"))
	name = strdup(kvp->val);
    else if (!strcmp(key, "WindowClass"))
	class = strdup(kvp->val);
    else if (!strcmp(key, "WindowOpacity"))
	opacity = f_val;
    else if (!strcmp(key, "DimOpacity"))
	dimopacity = f_val;
    else if (!strcmp(key, "DimColour") || !strcmp(key, "DimColor"))
	dimcolor = strdup(kvp->val);
    else if (!strcmp(key, "UnderlineColour") || !strcmp(key, "UnderlineColor"))
	undercolor = strdup(kvp->val);
    else if (!strcmp(key, "Prompt"))
	prompt = strdup(kvp->val);
    else if (!strcmp(key, "Font"))
	font = strdup(kvp->val);
    else if (!strcmp(key, "NormalBackground"))
	normbgcolor = strdup(kvp->val);
    else if (!strcmp(key, "NormalForeground"))
	normfgcolor = strdup(kvp->val);
    else if (!strcmp(key, "SelectedBackground"))
	selbgcolor = strdup(kvp->val);
    else if (!strcmp(key, "SelectedForeground"))
	selfgcolor = strdup(kvp->val);
    else if (!strcmp(key, "EmptyText"))
	emptyprompt = strdup(kvp->val);
    else if (!strcmp(key, "BorderColour") || !strcmp(key, "BorderColor"))
	bordercolor = strdup(kvp->val);
    else if (!strcmp(key, "BorderWidth"))
	borderwidth = i_val;
    else if (!strcmp(key, "SelectOnly"))
        selectonly = !!i_val;
    else if (!strcmp(key, "NoCaret"))
	nocaret = !!i_val;
}

void load_rc(const char* filename)
{
...
main(int argc, char *argv[]) {
	...
	if(!strcmp(argv[i], "-v")) {      /* prints version information */
	    ...
	    centerY = True;
	else if (!strcmp(argv[i], "-dynheight"))
	    dynheight = True;
	else if (!strcmp(argv[i], "-selectonly"))
	    selectonly = True;
	else if (!strcmp(argv[i], "-nocaret"))
	    nocaret = True;
...
/* Load the rc file last, so that the options */
/* in it take priority */
    if (rcfile != NULL)
	load_rc(rcfile);
...
drawmenu(void) {
    ...
    if(!nocaret && (curpos = textnw(dc, maskin ? maskinput : text, length) + dc->font.height/2) < dc->w && !selectonly)
    ...
    /* draw the empty prompt */
    if (nitems == 0 && emptyprompt) {
	dc->y += dc->h;
	drawtext(dc, emptyprompt, normcol);
    }

    /* resize window as necessary */
    if (lines > 0 && dynheight)
	XResizeWindow(dc->dpy, win, dc->w, dc->y + dc->h);
...
setup(void) {
     ...
     * need to center */
    if (centerX && mw != 0) {
	x = ((dimw / 2) - (mw / 2)) - borderwidth;
    }

    if (centerY) {
	y = (dimh - (lines * line_height)) / 2;
    }
    ...
    if (borderwidth > 0)
    {
	XSetWindowBorderWidth(dc->dpy, win, borderwidth);
	XSetWindowBorder(dc->dpy, win, bordercol->FG);
    }
